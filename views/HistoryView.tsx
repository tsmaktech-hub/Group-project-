...
import { getSessionsByLecturer, getRecordCountForSession } from '../services/supabase';

interface HistoryViewProps {
  onLogout: () => void;
}

const SessionCard: React.FC<{ session: AttendanceSession }> = ({ session }) => {
  const [count, setCount] = useState<number | null>(null);
  const course = COURSES.find(c => c.id === session.courseId);

  useEffect(() => {
    getRecordCountForSession(session.id).then(setCount);
  }, [session.id]);

  return (
    <div className="bg-white rounded-2xl shadow-sm border overflow-hidden hover:shadow-md transition-shadow flex flex-col">
      ...
      <div className="p-5 flex-1 bg-gray-50/50 flex items-center justify-between">
        <div>
          ...
          <p className="text-2xl font-black text-gray-800">{count !== null ? count : '...'}</p>
        </div>
...
export const HistoryView: React.FC<HistoryViewProps> = ({ onLogout }) => {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      getSessionsByLecturer(user.id).then(data => {
        setSessions(data);
        setLoading(false);
      });
    }
